...
class JTableSubscriber extends JTable
{
    ...
    public function normalized()
    {
        ...
        $item->registerDate = date('c', strtotime($item->registerDate));
    ...
    function check()
    {
        ...
        $query = 'SELECT id' . ' FROM #__subscribers ' . ' WHERE email = ' . $this->_db->Quote($this->email) . ' AND id <> ' . $this->_db->Quote((int)$this->id);
        ...
        if ($xid /*&& $xid != intval($this->id)*/) {
            $error = new stdClass();
            $error->key = 'email';
            $error->msg = 'Email already subscribed';
            return false;
        }
